...
export class SessionService {
    ...
    isLogged(){
      return this.http.get(`${BASEURL}/api/currentuser`,this.options).pipe(
        map( (res:Response) => {
          this.user = res.json();
    ...
    signup(username:string, password:string, name:string, secret:string): Observable<object>{
      return this.http.post(`${BASEURL}/api/signup`,{username,password,name,secret},this.options).pipe(map( (res:Response) => {
          let data = res.json();
          this.user = data.user;
          return this.user;
        }),
    ...
    logout(){
      return this.http.get(`${BASEURL}/api/logout`,this.options).pipe(map( (res:Response) => {
          this.user = null;
        }),
        catchError( e => of(this.errorHandler(e)))
      )
